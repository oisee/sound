...
def save_grouped_frames(grouped_frames, output_file_path):
    """Saves the grouped frames to a TSV file."""
...
def reconstruct_wav(input_folder, output_folder, waveform='cosine', duty=0.5):
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".tsv"):
            ...
            # Save the grouped frames to a TSV file
            grouped_tsv_filename = f"{os.path.splitext(filename)[0]}_grouped.tsv"
            grouped_tsv_file_path = os.path.join(output_folder, grouped_tsv_filename)
            save_grouped_frames(grouped_frames, grouped_tsv_file_path)
            print(f"Grouped frequencies saved to {grouped_tsv_file_path}")
